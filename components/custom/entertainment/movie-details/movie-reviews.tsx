...
import type { MovieReview } from "@/types/entertainment/movies/movie-reviews"
...
const formatDate = (dateStr: string) => {
  ...
  try {
    ...
    console.warn("Date format error in movie-reviews:", e)
    return dateStr
  }
}
...
export default function MovieReviewsSection({ movieId }: { movieId: string }) {
  ...
  const { data, isFetching, isError } = useFetchMovieReviews(movieId, tmdbPage)
  ...
  if (!showSkeleton && displayedReviews.length === 0 && reviews.length === 0) {
    const tmdbReviewUrl = `https://www.themoviedb.org/movie/${movieId}/reviews`
