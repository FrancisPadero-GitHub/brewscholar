...
// hooks
import { useFetchMovieRecommendations } from "@/hooks/entertainment/fetch/movies/useFetchMovieRecommendations"

// Movie card
function MovieRecommendationsCard({ movie }: { movie: MovieResult }) {
...
export default function MovieRecommendationsSection({ movieId }: { movieId: string }) {
  ...
  const { data, isFetching } = useFetchMovieRecommendations(movieId, page)
  ...
  // Reset page when movieId changes
  useEffect(() => {
    setPage(1)
  }, [movieId])

  const totalPagesRaw = data?.total_pages ?? 1
  const totalPagesFiltered = Math.min(totalPagesRaw, 10) // Limit recommendations to first 10 pages maximum
  const movies = data?.results ?? []

  // If there are no results and we are not fetching, do not render this section
      ...
      <div className="group/scroll relative">
        {isFetching ? (
          ...
          <>
            ...
            >
              {movies.map((movie) => (
                <MovieRecommendationsCard key={movie.id} movie={movie} />
              ))}
            </motion.div>
