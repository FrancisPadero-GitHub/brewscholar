...
// hooks
import { useFetchTvRecommendations } from "@/hooks/entertainment/fetch/tv-series/useFetchTvRecommendations"
...
export default function TvRecommendationsSection({
  ...
  const { data, isFetching } = useFetchTvRecommendations(seriesId, page)
  ...
  // Reset page when seriesId changes
  useEffect(() => {
    setPage(1)
  }, [seriesId])
  ...
  if (!isFetching && tvShows.length === 0) {
    return null
  }
      ...
      <div className="group/scroll relative">
        {isFetching ? (
          ...
          <>
            ...
            >
              {tvShows.map((tvShow) => (
                <TvRecommendationsCard key={tvShow.id} tvShow={tvShow} />
              ))}
            </motion.div>
